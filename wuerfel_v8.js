...
//************************************************************//
	...
	function designWuerfel(wuerfelElement, value) {
		...
		augePosition[value].forEach(pos => {					//je nachdem welche Zahl, dann rufe dazugehörige Augen auf
		 																	//für jedes ???
...
//***************************************************//
	...
	// Würfeln/randomizing nur für nicht gelockte
	function wuerfeln() {
	
		for (let i = 0; i < 5; i++) {												//gehe alle 0-4 Würfel durch im Array locked
			if (!locked[i]) {															//wenn bei locked nicht auf true, dann:
			...
			designWuerfel(w, werte[i]);											//erstelle Design jeweiliger Würfel
		}
	}
